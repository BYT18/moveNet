...
def draw_nonfigure_prediction(image, keypoints_with_scores, ground, crop_region=None, close_figure=False, output_image_height=None):
   """Draws the keypoint predictions on image"""
   height, width, channel = image.shape
   aspect_ratio = float(width) / height
   fig, ax = plt.subplots(figsize=(12 * aspect_ratio, 12))
...
if "tflite" in model_name:
   ...
   if "movenet_lightning" in model_name:
     ...
     module = hub.load("https://kaggle.com/models/google/movenet/TensorFlow2/singlepose-lightning/4")
     input_size = 192
   elif "movenet_thunder" in model_name:
...
display_image = tf.expand_dims(image, axis=0)
display_image = tf.cast(tf.image.resize_with_pad(display_image, 1200, 2132), dtype=tf.int32)
